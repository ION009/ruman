...
export async function GET(request: Request) {
  ...
  if (isControlPlaneEnabled()) {
    try {
      ...
      try {
        return Response.json(await withAnalyticsTokenFallback((token) => getDashboardErrorsData(site.id, range, token)));
      } catch (error) {
  ...
  try {
    return Response.json(await getDashboardErrorsData(requestedSiteId, range, token));
  } catch (error) {
