...
export async function GET(request: Request) {
  ...
  if (isControlPlaneEnabled()) {
    try {
      ...
      try {
        return Response.json(await withAnalyticsTokenFallback((token) => getDashboardPerformanceData(site.id, range, token)));
      } catch (error) {
  ...
  try {
    return Response.json(await getDashboardPerformanceData(requestedSiteId, range, token));
  } catch (error) {
