...
export async function GET(request: Request) {
      ...
      try {
        return Response.json(
          await withAnalyticsTokenFallback((token) =>
            getDashboardRetentionTrendData(site.id, range, { cadence, device, country }, token),
          ),
        );
      } catch (error) {
        return Response.json(
  ...
  try {
    return Response.json(await getDashboardRetentionTrendData(requestedSiteId, range, { cadence, device, country }, token));
  } catch (error) {
