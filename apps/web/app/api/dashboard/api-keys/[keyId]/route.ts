...
import { deleteAPIKey } from "@/lib/control-plane/api-keys";
...
export async function DELETE(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    await deleteAPIKey(site.id, keyId);
    return Response.json({ ok: true });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Failed to delete API key." }, { status: 400 });
  }
}
