...
import {
  ...
  getDashboardReplaySessionsData,
  readDashboardToken,
...
export async function GET(request: Request) {
    ...
    try {
      ...
      try {
        return Response.json(
          await withAnalyticsTokenFallback((token) => getDashboardReplaySessionsData(site.id, range, token)),
        );
      } catch (error) {
        return Response.json(
  ...
  try {
    return Response.json(await getDashboardReplaySessionsData(requestedSiteId, range, token));
  } catch (error) {
