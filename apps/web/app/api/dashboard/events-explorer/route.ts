...
export async function GET(request: Request) {
    ...
    try {
      ...
      try {
        return Response.json(
          await withAnalyticsTokenFallback((token) => getDashboardEventExplorerData(site.id, range, token)),
        );
      } catch (error) {
        return Response.json(
  ...
  try {
    return Response.json(await getDashboardEventExplorerData(requestedSiteId, range, token));
  } catch (error) {
