...
export async function GET(request: Request) {
      ...
      try {
        return Response.json(
          await withAnalyticsTokenFallback((token) =>
            getDashboardJourneysData(site.id, range, { device, country, limit }, token),
          ),
        );
      } catch (error) {
        return Response.json(
  ...
  try {
    return Response.json(await getDashboardJourneysData(requestedSiteId, range, { device, country, limit }, token));
  } catch (error) {
